        ...
        builder: (context, ref, _) {
          ...
          return currenciesAsync.when(
            data: (currencies) {
              ...
              if (active.isEmpty) {
                return const Center(
                  child: Text('No hay monedas activas', style: TextStyle(color: AppColors.onSurfaceVariant)),
                );
              }

              // Ensure base currency is valid
              if (!active.any((c) => c.currency.code == _baseCurrencyCode)) {
                _baseCurrencyCode = active.first.currency.code;
              }

              final baseCurrency = active.firstWhere((c) => c.currency.code == _baseCurrencyCode);
              final baseUnitsPerUsd = baseCurrency.unitsPerUsd.toDouble();
              final inputAmount = double.tryParse(_amountController.text) ?? 0.0;
              final amountInUsd = baseUnitsPerUsd > 0 ? inputAmount / baseUnitsPerUsd : 0.0;
              ...
              return ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        icon: const Icon(Icons.settings, color: AppColors.wealth),
                        tooltip: 'Administrar Monedas',
                                ...
                                Container(
                                  ...
                                  child: DropdownButtonHideUnderline(
                                    child: DropdownButton<String>(
                                      ...
                                      icon: const Icon(Icons.keyboard_arrow_down, color: AppColors.onSurface, size: 18),
                                      ...
                                      onChanged: (String? newValue) {
                                        if (newValue != null) {
                                          setState(() {
                                            _baseCurrencyCode = newValue;
                                          });
                                        }
                                      },
                                      items: active.map<DropdownMenuItem<String>>((c) {
                                        return DropdownMenuItem<String>(
                                          value: c.currency.code,
                                          child: Text(c.currency.code),
                                        );
                                      }).toList(),
                                    ),
                                  ),
                  ...
                  // Conversions
                  ...active.where((c) => c.currency.code != _baseCurrencyCode).map((currency) {
                    final convertedAmount = amountInUsd * currency.unitsPerUsd.toDouble();
                    return Padding(
